...
const RoleBasedRoute = ({ allowedRoles }: RoleBasedRouteProps) => {
  ...
  const userRoles = user?.roles?.map((role) => role.replace("ROLE_", "").toUpperCase()) || [];

  // Verifica se o usuário tem pelo menos um dos perfis permitidos
  const isAuthorized = userRoles.some((role) => allowedRoles.map((r) => r.toUpperCase()).includes(role));
